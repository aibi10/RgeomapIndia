...
server <- function(input, output) {
  ...
  map = leaflet(data = data_map)%>%
    ...
    addMarkers(lng = Power_Plants$Longitude, lat = Power_Plants$Latitude,clusterOptions = markerClusterOptions(),label = ~lapply(Power_Plants$table, htmltools::HTML),group = "Thermal Power Plants",icon = ~icons(iconUrl = "https://www.freeiconspng.com/uploads/green-normal-triangle-png-8.png",iconWidth = 18, iconHeight = 18))%>%
